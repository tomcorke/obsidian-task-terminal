...
export class TaskMover {
  ...
  async moveTask(file: TFile, newColumn: KanbanColumn): Promise<void> {
    const content = await this.app.vault.read(file);
    const newState = newColumn;
    ...
    updated = updated.replace(oldTagPattern, `$1${newState}`);
    ...
    const logEntry = `- **${dateStr}** - Moved to ${newState} (via kanban board)`;

    const logIndex = updated.indexOf("## Activity Log");
    if (logIndex !== -1) {
      // Find the end of the activity log section (next ## or end of file)
